...
matrix *matrix_new(unsigned int num_rows, unsigned int num_cols) {
  if (num_rows == 0) {
    MAT_ERROR(INVALID_ROWS);
    return NULL;
  }
  if (num_cols == 0) {
    MAT_ERROR(INVALID_COLS);
    return NULL;
  }
...
matrix *matrix_fromfile(const char *file) {
  ...
  if (NULL == m_file) {
    MAT_FERROR(CANNOT_OPEN_FILE, file);
    return NULL;
  }
  matrix *r = matrix_fromfilef(m_file);
  fclose(m_file);
  return r;
}
...
matrix *matrix_col_get(matrix *m, unsigned int col) {
  if (col >= m->num_cols) {
    MAT_FERROR(CANNOT_GET_COLUMN, col, m->num_cols);
    return NULL;
  }
...
matrix *matrix_row_get(matrix *m, unsigned int row) {
  if (row >= m->num_rows) {
    MAT_FERROR(CANNOT_GET_ROW, row, m->num_rows);
    return NULL;
  }
...
int matrix_diag_set(matrix *m, double value) {
  if (!m->is_square) {
    MAT_FERROR(CANNOT_SET_DIAG, value);
    return 0;
  }
  int i;
  for(i = 0; i < m->num_rows; i++) {
...
int matrix_row_mult_r(matrix *m, unsigned int row, double num) {
  if (row>= m->num_rows) {
    MAT_FERROR(CANNOT_ROW_MULTIPLY, row, m->num_rows);
    return 0;
  }
  int i;
...
int matrix_col_mult_r(matrix *m, unsigned int col, double num) {
  if (col>=m->num_cols) {
    MAT_FERROR(CANNOT_COL_MULTIPLY, col, m->num_cols);
    return 0;
  }
  int i;
  for(i = 0; i < m->num_rows; i++) {
...
matrix *matrix_row_addrow(matrix *m, unsigned int where, unsigned int row, double multiplier) {
  matrix *r = matrix_cp(m);
  if (!matrix_row_addrow_r(m, where, row, multiplier)) {
    matrix_free(r);
    return NULL;
  }
  return r;
...
int matrix_row_addrow_r(matrix *m, unsigned int where, unsigned int row, double multiplier) {
  ...
  if (where >= m->num_rows || row >= m->num_rows) {
    MAT_FERROR(CANNOT_ADD_TO_ROW, multiplier, row, where, m->num_rows);
    return 0;
  }
  int i = 0;
  for(i = 0; i < m->num_cols; i++) {
...
matrix *matrix_col_rem(matrix *m, unsigned int column) {
  if(column >= m->num_cols) {
    MAT_FERROR(CANNOT_REMOVE_COLUMN, column, m->num_cols);
    return NULL;
  }
...
matrix *matrix_row_rem(matrix *m, unsigned int row) {
  if (row >= m->num_rows) {
    MAT_FERROR(CANNOT_REMOVE_ROW, row, m->num_rows);
    return NULL;
  }
...
int matrix_row_swap_r(matrix *m, unsigned int row1, unsigned int row2) {
  if (row1 >= m->num_rows || row2 >= m->num_rows) {
    MAT_FERROR(CANNOT_SWAP_ROWS, row1, row2, m->num_rows);
    return 0;
  }
...
int matrix_col_swap_r(matrix *m, unsigned int col1, unsigned int col2) {
  if (col1 >= m->num_cols || col2 >= m->num_cols) {
    MAT_FERROR(CANNOT_SWAP_ROWS, col1, col2, m->num_cols);
    return 0;
  }
  double tmp;
  int j;
...
matrix *matrix_cath(unsigned int mnum, matrix **marr) {
  if (0==mnum) {
    return NULL;
  }
  if (1==mnum) {
  ...
  for(k = 1; k < mnum; k++) {
    if (NULL == marr[k]) {
      MAT_FERROR(INCONSITENT_ARRAY, k, mnum);
      return NULL;
    }
    if (lrow != marr[k]->num_rows) {
      MAT_FERROR(CANNOT_CONCATENATE_H, lrow, marr[k]->num_rows);
      return NULL;
    }
  ...
  matrix *r = matrix_new(lrow, ncols);
  for(i = 0; i < r->num_rows; i++) {
    k = 0;
    offset = 0;
    for(j = 0; j < r->num_cols; j++) {
...
// be variable
matrix *matrix_catv(unsigned int mnum, matrix **marr) {
  if (0 == mnum) {
    return NULL;
  }
  if (1 == mnum) {
  ...
  for(i = 0; i < mnum; i++) {
    if (NULL==marr[i]) {
      MAT_FERROR(INCONSITENT_ARRAY, i, mnum);
      return NULL;
    }
    if (lcol != marr[i]->num_cols) {
      MAT_FERROR(CANNOT_CONCATENATE_V,lcol,marr[i]->num_cols);
      return NULL;
    }
...
matrix *matrix_transp(matrix *m) {
  int i, j;
  matrix *r = matrix_new(m->num_cols, m->num_rows);
  for(i = 0; i < r->num_rows; i++) {
    for(j = 0; j < r->num_cols; j++) {
...
int _matrix_pivotidx(matrix *m, unsigned int col, unsigned int row) {
...
int _matrix_pivotmaxidx(matrix *m, unsigned int col, unsigned int row) {
  int i, maxi;
  double micol;
...
matrix *matrix_rref(matrix *m) {
  ...
  while(j < r->num_cols && i < r->num_rows) {
    ...
    pivot = _matrix_pivotmaxidx(r, j, i);
    if (pivot<0) {
...
int _matrix_absmaxr(matrix *m, unsigned int k) {
...
void matrix_lup_printf(matrix_lup *lu, const char *fmt) {
  matrix_printf(lu->L, fmt);
  matrix_printf(lu->U, fmt);
  matrix_printf(lu->P, fmt);
}

matrix_lup *matrix_lup_solve(matrix *m) {
  if (!m->is_square) {
    MAT_FERROR(CANNOT_LU_MATRIX_SQUARE, m->num_rows, m-> num_cols);
    return NULL;
  }
  matrix *L = matrix_new(m->num_rows, m->num_rows);
  matrix *U = matrix_cp(m);
  matrix *P = matrix_eye(m->num_rows);
  ...
  for(j = 0; j < U->num_cols; j++) {
    ...
    if (pivot!=j) {
      ...
      matrix_row_swap_r(U, j, pivot);
      matrix_row_swap_r(L, j, pivot);
      matrix_row_swap_r(P, j, pivot);
    ...
    for(i = j+1; i < U->num_rows; i++) {
      ...
      matrix_row_addrow_r(U, i, j, -mult);
      // Store the multiplier in L
      L->data[i][j] = mult;
    }
  }
  matrix_diag_set(L, 1.0);

  return matrix_lup_new(L, U, P, num_permutations);
}
...
matrix *matrix_inv(matrix_lup *lup) {
  unsigned n = lup->L->num_cols;
  matrix *r = matrix_sqr(n);
  matrix *I = matrix_eye(lup->U->num_rows);
  matrix *invx;
  matrix *Ix;
  int i,j;
  for(j =0; j < n; j++) {
...
double nml_vect_dot(matrix *m1, unsigned int m1col, matrix *m2, unsigned m2col) {
  ...
  if (m1col >= m1->num_cols) {
    MAT_FERROR(CANNOT_GET_COLUMN, m1col, m1->num_cols);
  }
  if (m2col >= m2->num_cols) {
    MAT_FERROR(CANNOT_GET_COLUMN, m2col, m2->num_cols);
  }
  int i;
  double dot = 0.0;
  for(i = 0; i < m1->num_rows; i++) {
...
double matrix_col_l2norm(matrix *m, unsigned int col) {
  if (col >= m->num_cols) {
    MAT_FERROR(CANNOT_COLUMN_L2NORM, col, m->num_cols);
  }
...
int matrix_normalize_r(matrix *m) {
  ...
  for(j = 0; j < m->num_cols; j++) {
    ...
    matrix_col_mult_r(m, j, 1/l2norms->data[0][j]);
  }
  matrix_free(l2norms);
  return 1;
}
...
matrix_qr *matrix_qr_solve(matrix *m) {
  ...
  matrix_qr *qr = matrix_qr_new();
  matrix *Q = matrix_cp(m);
  matrix *R = matrix_new(m->num_rows, m->num_cols);
  ...
  for(j=0; j < m->num_cols; j++) {
    ...
    l2norm = matrix_col_l2norm(Q, j);
    matrix_col_mult_r(Q, j, 1/l2norm);
    R->data[j][j] = l2norm;
    matrix_free(aj);
  }
  qr->Q = Q;
